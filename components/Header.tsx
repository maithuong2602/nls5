...
import { Layers, Download, FileText, Layout, FileType, BarChart3, Presentation, Upload, Database } from 'lucide-react';
...
interface HeaderProps {
    ...
    onExportWord: () => void;
    onExportJson: (scope: 'subject' | 'grade') => void;
    onImportJson: () => void;
    onOpenMatrix: () => void;
}
...
const Header: React.FC<HeaderProps> = ({ 
    ...
    onExportWord, onExportJson, onImportJson,
    onOpenMatrix
}) => {
    return (
            ...
            <div className="flex items-center gap-3">
                ...
                <div className="flex gap-2">
                    <button 
                        ...
                        className="flex items-center gap-2 bg-amber-500 hover:bg-amber-600 text-white px-3 py-2 rounded-lg shadow transition active:scale-95 text-[10px] font-bold uppercase"
                        title="Nhập dữ liệu từ file JSON (Trộn vào dữ liệu hiện có)"
                    >
                        <Upload size={14} /> Nhập
                    </button>
                    <button 
                    ...
                    <div className="flex bg-slate-800 rounded-lg overflow-hidden shadow">
                        <button 
                            onClick={() => onExportJson('grade')}
                            className="flex items-center gap-2 hover:bg-slate-700 text-white px-3 py-2 border-r border-slate-700 transition active:scale-95 text-[10px] font-bold uppercase"
                            title="Xuất JSON chỉ riêng Khối hiện tại"
                        >
                            <Download size={14} /> Khối
                        </button>
                        <button 
                            onClick={() => onExportJson('subject')}
                            className="flex items-center gap-2 hover:bg-slate-700 text-white px-3 py-2 transition active:scale-95 text-[10px] font-bold uppercase"
                            title="Xuất JSON toàn bộ môn học"
                        >
                            <Database size={14} /> Môn
                        </button>
                    </div>
                </div>
            </div>
