...
import { Competency, Lesson, Subject, Topic, Mappings } from '../types';
import { Plus, X, Search, Sparkles, Wand2, Loader2, Upload, FileText, Info, Scissors, Merge, BookOpen, Check, History, MapPin, MonitorPlay, AlertTriangle } from 'lucide-react';
...
interface CompetencyTableProps {
    ...
    onBulkApplyMappings?: (mappings: Mappings) => void;
    onSuggestAI: () => void;
...
const CompetencyTable: React.FC<CompetencyTableProps> = ({
    ...
    const handleAiSuggest = async () => {
        ...
        try {
            ...
            // Construct concise list of competencies for prompt
            const compListStr = competencies.map(c => `${c.code}: ${c.text}`).join('\n');
            const roleContext = currentSubject === 'Tin học' ? 'Chủ đạo (Dạy kỹ năng cốt lõi)' : 'Tích hợp (Vận dụng kỹ năng số)';
            
            const prompt = `
                [VAI TRÒ CỦA BẠN]
                Bạn là một Chuyên gia Giáo dục và Chuyển đổi số hàng đầu tại Việt Nam, am hiểu sâu sắc về Chương trình GDPT 2018 và Khung năng lực số (NLS) dành cho người học theo Thông tư số 02/2025/TT-BGDĐT.

                [THÔNG TIN BÀI DẠY]
                - Môn học: ${currentSubject} (${roleContext})
                - Tên bài: "${lesson.title}"
                - Chủ đề: "${topicName}"
                - Yêu cầu cần đạt (YCCĐ): ${lesson.yccd.join('; ')}

                [NHIỆM VỤ]
                Hãy phân tích bài dạy và đề xuất tích hợp NLS theo quy trình 3 bước:
                1. Xác định địa chỉ tích hợp: Hoạt động nào trong bài dùng công nghệ?
                2. Chọn mã NLS: Chọn chính xác từ danh sách bên dưới. Ưu tiên các miền (1) Thông tin, (3) Sáng tạo, (5) Giải quyết vấn đề, (6) AI.
                3. Mô tả hành vi & Công cụ (QUAN TRỌNG):
                   - Nếu môn học KHÔNG PHẢI là Tin học: Nội dung minh chứng (reason) BẮT BUỘC phải nêu rõ tên "Công cụ" hoặc kỹ năng Tin học cụ thể mà học sinh sử dụng (ví dụ: "Sử dụng phần mềm Geogebra...", "Dùng công cụ tìm kiếm Google...", "Sử dụng bài trình chiếu PowerPoint..."). Minh chứng cần bám sát động từ hành động của YCCĐ.
                   - Nếu là môn Tin học: Mô tả hành động cụ thể của học sinh.

                [DANH SÁCH MÃ NĂNG LỰC KHẢ DỤNG]
                ${compListStr}

                [YÊU CẦU ĐẦU RA]
                Trả về kết quả dưới dạng JSON Array (Mảng đối tượng), không markdown.
                Cấu trúc: [{ "code": "Mã NLS", "reason": "Mô tả hành vi cụ thể (Ví dụ: HS sử dụng phần mềm X để...)" }]
            `;
            ...
            let suggestedItems: { code: string, reason: string }[] = [];
            
            if (Array.isArray(parsedResponse)) {
                if (typeof parsedResponse[0] === 'string') {
                    suggestedItems = parsedResponse.map((c: string) => ({ code: c, reason: 'Gợi ý từ AI' }));
                } else {
                    suggestedItems = parsedResponse;
                }
            }
            
            const newMappings: Mappings = {};
            const missingITFoundation: string[] = [];

            suggestedItems.forEach(item => {
                const { code, reason } = item;
                const isValidCode = competencies.some(c => c.code === code);
                if (isValidCode) {
                    const isCoveredInIT = availableITCompetencies[code] && availableITCompetencies[code].length > 0;
                    
                    if (currentSubject === 'Tin học' || isCoveredInIT) {
                        // Nếu là môn Tin học hoặc đã có nền tảng CNTT -> Auto select
                        newMappings[code] = {
                            selected: true,
                            type: 'suggested',
                            reason: reason // Sử dụng minh chứng cụ thể do AI tạo
                        };
                    } else {
                        // Nếu không có nền tảng CNTT -> Cảnh báo
                        missingITFoundation.push(code);
                    }
                }
            });

            // Apply valid mappings
            if (Object.keys(newMappings).length > 0 && onBulkApplyMappings) {
                onBulkApplyMappings(newMappings);
            }

            // Show Result Notification
            if (missingITFoundation.length > 0) {
                alert(`AI đã tìm thấy ${suggestedItems.length} năng lực phù hợp.\n\n` +
                      `✅ Đã tự động chọn ${Object.keys(newMappings).length} năng lực (có nền tảng CNTT/Tin học).\n\n` +
                      `⚠️ CẢNH BÁO: Các năng lực sau phù hợp nhưng học sinh CHƯA được học ở môn Tin học tính đến Tuần ${currentLessonStartWeek}:\n` +
                      `- ${missingITFoundation.join('\n- ')}\n\n` +
                      `Giáo viên cần cân nhắc tổ chức hoạt động bổ trợ nếu muốn tích hợp các năng lực này.`);
            } else if (Object.keys(newMappings).length > 0) {
                 alert(`✅ AI đã đề xuất và cập nhật minh chứng cho ${Object.keys(newMappings).length} năng lực phù hợp theo Thông tư 02/2025.`);
            } else {
                alert("AI không tìm thấy năng lực nào phù hợp rõ ràng cho bài học này.");
            }

        } catch (error) {
            console.error("AI Suggest Error", error);
            alert("Lỗi khi gọi AI gợi ý. Vui lòng thử lại.");
        } finally {
    ...
    return (
        <div className="flex-grow flex flex-col bg-slate-50/50 relative overflow-hidden h-full">
            <LoadingOverlay isVisible={isAiLoading || localIsAnalyzing} message={localIsAnalyzing ? "AI đang đối chiếu TT 02/2025..." : "Hệ thống đang xử lý..."} />
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
                    <div className="bg-white rounded-xl shadow-2xl w-full max-w-lg flex flex-col">
                        <div className="p-4 border-b border-slate-100 flex justify-between items-center">
                            <h3 className="font-bold text-lg text-slate-800">Nhập Yêu cầu cần đạt</h3>
                            <button onClick={() => setShowImportModal(false)}><X size={20}/></button>
                        </div>
                        <div className="p-4">
                            <textarea 
                                className="w-full h-40 border border-slate-300 rounded p-2 text-sm outline-none" 
                                placeholder="Dán danh sách YCCĐ vào đây (mỗi dòng một yêu cầu)..."
                                value={importText}
                                onChange={e => setImportText(e.target.value)}
                            />
                        </div>
                        <div className="p-4 border-t border-slate-100 bg-slate-50 flex justify-end gap-2">
                            <button onClick={processImport} className="bg-teal-600 text-white px-4 py-2 rounded text-sm font-bold">Xác nhận nhập</button>
                        </div>
                    </div>
                </div>
            )}
                ...
                <div className="flex flex-col gap-4">
                    ...
                    <div className="flex items-center justify-between gap-4">
                        ...
                        <div className="flex items-center gap-3">
                            ...
                            <button onClick={handleAiSuggest} disabled={isAiLoading || localIsAnalyzing} className="flex items-center gap-2 bg-gradient-to-r from-violet-600 to-indigo-600 text-white px-4 py-1.5 rounded-full shadow-md transition-all active:scale-95 disabled:opacity-50 hover:shadow-lg">
