...
const CompetencyTable: React.FC<CompetencyTableProps> = ({
    ...
    const handleAiSuggest = async () => {
        ...
        try {
            ...
            const prompt = `
                Bạn là chuyên gia giáo dục số. Hãy phân tích bài học môn ${currentSubject} sau:
                - Tên bài: "${lesson.title}"
                - Chủ đề: "${topicName}"
                - Yêu cầu cần đạt (YCCĐ): ${lesson.yccd.join('; ')}

                Nhiệm vụ:
                1. Chọn các mã năng lực số phù hợp nhất để tích hợp, dựa trên danh sách dưới đây.
                2. Với mỗi năng lực được chọn, viết một câu "minh chứng" ngắn gọn (dưới 25 từ) mô tả học sinh thực hiện hoạt động gì trong bài học này để đạt được năng lực đó. 
                   Lưu ý: Minh chứng phải bám sát vào YCCĐ của bài học này.
                   Ví dụ minh chứng: "HS sử dụng phần mềm mô phỏng để quan sát..." hoặc "HS truy cập internet để tìm kiếm thông tin về..."

                Danh sách năng lực:
                ${compListStr}

                Trả về kết quả dưới dạng JSON là một mảng các object: [{ "code": "...", "reason": "..." }]
            `;
            ...
            if (missingITFoundation.length > 0) {
                alert(`AI đã tìm thấy ${suggestedItems.length} năng lực phù hợp.\n\n` +
                 ...
                 alert(`✅ AI đã đề xuất và cập nhật minh chứng cho ${Object.keys(newMappings).length} năng lực phù hợp.`);
            } else {
    ...
    return (
        <div className="flex-grow flex flex-col bg-slate-50/50 relative overflow-hidden h-full">
            <LoadingOverlay isVisible={isAiLoading || localIsAnalyzing} message={localIsAnalyzing ? "AI đang đối chiếu chương trình Tin học..." : "Hệ thống đang xử lý..."} />
