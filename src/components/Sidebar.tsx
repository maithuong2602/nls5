...
// ----------------------------------------------------------------------
// BIẾN MODULE (Global trong file này) - ĐẢM BẢO ỔN ĐỊNH TUYỆT ĐỐI
// ----------------------------------------------------------------------
// Lưu trữ thông tin vật thể đang kéo bên ngoài React LifeCycle
// để tránh việc Re-render làm mất dữ liệu kéo.
...
const Sidebar: React.FC<SidebarProps> = ({ 
    ...
    // State chỉ dùng để vẽ đường kẻ xanh (Visual Feedback)
    ...
    const handleDragStart = (e: React.DragEvent, type: 'lesson' | 'topic', id: string | number) => {
        ...
        // Một số trình duyệt (Firefox) yêu cầu phải có dữ liệu mới cho kéo
        e.dataTransfer.setData("text/plain", JSON.stringify({ type, id }));

        // Kỹ thuật quan trọng: Thay đổi UI (làm mờ) SAU khi trình duyệt đã chụp ảnh drag
        // Nếu làm ngay lập tức, drag có thể bị hủy
    ...
    const handleDragOverLesson = (e: React.DragEvent, lessonId: string | number) => {
        e.preventDefault(); // BẮT BUỘC: Nếu không có dòng này, sự kiện Drop sẽ không bao giờ chạy
        e.stopPropagation();
        ...
        // Chỉ setState khi thực sự thay đổi để giảm re-render
    ...
    const handleDrop = (e: React.DragEvent, targetTopicIndex: number, targetLessonIndex?: number) => {
        ...
        if (activeDragPayload.type === 'lesson') {
            ...
            if (targetLessonIndex !== undefined) {
                // Drop vào Lesson khác
                // Tính toán lại vị trí một lần nữa cho chính xác tuyệt đối
                const rect = (e.currentTarget as HTMLElement).getBoundingClientRect();
                const midY = rect.top + rect.height / 2;
                const position = e.clientY < midY ? 'top' : 'bottom';
                ...
                onReorderLesson(sourceId, targetTopicIndex, insertIndex);
                 ...
                 // Drop vào Header Topic -> chèn vào đầu
                ...
                {topics.map((topic, originalIdx) => {
                    ...
                    return (
                        <div 
                            key={originalIdx} 
                            className="relative"
                            onDragOver={(e) => e.preventDefault()} // Ngăn chặn sự kiện mặc định trên container
                        >
                            {/* --- Topic Drop Indicators (Always rendered, toggled via opacity) --- */}
                            <div className={`absolute top-0 left-0 right-0 h-1 bg-teal-500 z-30 shadow-[0_0_8px_#14b8a6] pointer-events-none transition-opacity duration-100 ${dropPos === 'top' ? 'opacity-100' : 'opacity-0'}`}></div>
                            <div className={`absolute bottom-0 left-0 right-0 h-1 bg-teal-500 z-30 shadow-[0_0_8px_#14b8a6] pointer-events-none transition-opacity duration-100 ${dropPos === 'bottom' ? 'opacity-100' : 'opacity-0'}`}></div>
                            ...
                            <div 
                                ...
                                className={`px-4 py-2 flex justify-between items-center group cursor-grab active:cursor-grabbing transition-colors border-t border-transparent relative select-none
                                    ${isTopicDropTarget && dropPos === 'inside' ? 'bg-teal-50 border-teal-500 border-dashed border' : ''}
                                    ${!isTopicDropTarget ? 'hover:bg-slate-100' : ''}`}
                            >
                            ...
                            <div className={`transition-all ${isTopicDropTarget && dropPos === 'inside' ? 'bg-teal-50/50 pb-2' : ''}`}>
                                {topic.lessons.map((lesson, lessonIndex) => {
                                    ...
                                    return (
                                        ...
                                        >
                                            {/* Lesson Drop Indicators */}
                                            <div className={`absolute top-0 left-0 right-0 h-0.5 bg-teal-500 z-30 shadow-[0_0_4px_#14b8a6] pointer-events-none transition-opacity duration-75 ${dropPosLesson === 'top' ? 'opacity-100' : 'opacity-0'}`}></div>
                                            <div className={`absolute bottom-0 left-0 right-0 h-0.5 bg-teal-500 z-30 shadow-[0_0_4px_#14b8a6] pointer-events-none transition-opacity duration-75 ${dropPosLesson === 'bottom' ? 'opacity-100' : 'opacity-0'}`}></div>
                                            ...
                                            <div className={`flex flex-col gap-1 items-end justify-start py-2 pr-2 shrink-0 z-10 transition-opacity duration-200 ${isActive ? 'opacity-100' : 'opacity-0 group-hover:opacity-100'} bg-white/50 backdrop-blur-[1px] rounded-l`}>
                                ...
                                {/* Droppable Area at the end of lesson list */}
